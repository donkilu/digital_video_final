...
int main(int argc, char** argv)
{
	if(argc >= 3)
	{
        ...
        // find circle+HSV in the video
        int found=0;
        cv::Point* previous_frame_center = new Point[10];
        ...
        for(int frame_i=0; frame_i < inputVideo.get(CAP_PROP_FRAME_COUNT); ++frame_i)
        {
            ...
            cv::Point* HSV_center = new Point[balls.size()];
            // draw candidates
            ...
            if(frame_i>39){
                for (int j = 9; j > 0; j--)
                {
                    previous_frame_center[j] = previous_frame_center[j - 1];
                }
            }
            ...
            for (size_t ii = 0; ii < balls.size(); ii++)
            {
                //
                cv::Point center;
                center.x = ballsBox[ii].x + ballsBox[ii].width / 2;
                center.y = ballsBox[ii].y + ballsBox[ii].height / 2;
                cv::circle(result, center, 2, CV_RGB(20,150,20), -1);
                HSV_center[ii] = center;
            ...
            double dis_previous_min = 1000;
            double dis_previous;
            int dis_previous_index=0;
            if( frame_i>50 && point_history_tmp[0].size()<1){
                double avg_x=0, avg_y=0;
                for (size_t k = 1; k < 10; k++){
                    avg_x += previous_frame_center[k].x;
                    avg_y += previous_frame_center[k].y;
                }
                avg_x=avg_x/9;
                avg_y=avg_y/9;
                cv::Point previous_frame_avg_center;
                previous_frame_avg_center.x =avg_x;
                previous_frame_avg_center.y =avg_y;
                
                for (size_t i = 0; i < balls.size(); i++)
                {
                    dis_previous = cv::norm(HSV_center[i]-previous_frame_avg_center);
                    if(dis_previous<dis_previous_min){
                        dis_previous_min=dis_previous;
                        dis_previous_index = i;
                    }

                }
                previous_frame_center[0] =HSV_center[dis_previous_index];
                drawContours(result, balls, dis_previous_index, CV_RGB(255,255,128), 5);
            }else{
                previous_frame_center[0] = final_center;
                stringstream sstr;
